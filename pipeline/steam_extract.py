...
if __name__ == "__main__":
    ...
    for each_container in all_containers:
        title = each_container.find('span', class_="title").text
        price = each_container.find('div', class_="discount_final_price").text
        release_date = each_container.find(
            'div', class_="col search_released responsive_secondrow").text
        game_url = each_container['href']
        ...
        game.append([title, price, release_date.strip(), game_url])
        game_list = [game, platform_list, rating, developer]
